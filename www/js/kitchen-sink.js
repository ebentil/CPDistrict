...
myApp.onPageInit('login-screen-embedded', function (page) {
    $$(page.container).find('.button').on('click', function () {
        if (!username || !password){
            myApp.alert('Please fill in all form fields');
            return;
        }
        var username = $$(page.container).find('input[name="username"]').val();
        var password = $$(page.container).find('input[name="password"]').val();
        myApp.alert('Usernames: ' + username + ', password: ' + password, function () {
            //mainView.router.back();
            mainView.router.load('makepayment.html')
        });
...
$$('.login-screen').find('.button').on('click', function () {
    ...
    //myApp.alert('Usernamez: ' + username + ', password: ' + password, function () {
    ...
    $$.ajax({
        ...
        success : function(data,status,xhr) {
            ...
            if(data[0]==1){
                var collector_arr = data.split("|");
                var collector_id = collector_arr[1];
                var collector = collector_arr[2];
                var district = collector_arr[3];
                ...
                myApp.alert('Incorrect Username/Password');
            }
...
/* ===== Generate Payment History Content Dynamically ===== */
function createPayHistoryPage() {
    ...
    $$.ajax({
        ...
        success: function (data, status, xhr) {
            if(cat_level==4){
                ...
                var amount = data_arr[0];
                var rate = data_arr[1];
                $$('#amount').val(amount);
                $$('#rate').val(rate);
            }
            else{
                $$("#sub_category" + cat_level).html(data);
            }
...
myApp.onPageInit('makepayment',function(){
    ...
    $$('#sub_category1').on('change', function (e) {
        var cat_id = $$(this).val();
        var cat_level = 2;
        loadsub_cat(cat_id,cat_level);
    });
    $$('#sub_category2').on('change', function (e) {
        var cat_id = $$(this).val();
        var cat_level = 3;
        loadsub_cat(cat_id,cat_level);
    });
    $$('#sub_category3').on('change', function (e) {
        //load amount
        var cat_id = $$(this).val();
        var cat_level = 4;
        loadsub_cat(cat_id,cat_level);
